...
ci-docker:
	...
	docker run --rm --mount type=bind,source="$(CURDIR)",target=/workspace --workdir /workspace --env RENV_PATHS_LIBRARY=/tmp/up-r-library --env NOT_CRAN=true rocker/r-ver:4.6.0 sh -c 'apt-get update && apt-get install -y libxml2-dev libcurl4-openssl-dev libssl-dev libuv1-dev cmake pkg-config zlib1g-dev libicu-dev && Rscript -e '\''install.packages("renv"); renv::restore(prompt=FALSE)'\'' && Rscript -e '\''renv::load(); l <- c(lintr::lint_dir("scripts"), lintr::lint_dir("R")); print(l); stopifnot(length(l)==0); source("tests/test_standardized_release.R"); source("tests/test_election_panels.R"); source("tests/test_weaver_preparation.R"); source("tests/test_historical_lgd.R")'\'''
