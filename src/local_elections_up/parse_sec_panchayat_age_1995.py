...
# ///
"""Preserve the 1995 Panchayat age-percentage matrices, including interior blanks."""
...
SPECS = {
    "elec95_cp_age.pdf": {
        "sha": "6f206a8518fc45c084f26916371f42d6d5ce79e66f795cbd866b82af1662000a",
        "title": (
            "Details of Chairperson according to their age group in General Election -"
            " 1995"
        ),
        "posts": [
            ("I. Pradhan", "gp_head"),
    ...
    "elec95_mem_age.pdf": {
        "sha": "8ff0df5e51af2cef9851b79a524cee230b93c34a35ca90d0965abd7b3545357c",
        "title": (
            "Details of Members according to their age group in General Election - 1995"
        ),
        "posts": [
...
AGE = ["Upto 35 Years", "Between 35-50 years", "Avobe 50 Years"]
...
def parse_xml(xml, report, spec):
    ...
    anchors = [
        line
        for line in lines
        if " ".join(w["text"] for w in line).startswith("Scheduled Tribe ")
        and sum(bool(NUMBER.fullmatch(w["text"])) for w in line) == 9
    ]
    if not anchors:
        raise ValueError("No complete anchor row for nine numeric columns")
    ...
    for line in lines:
        ...
        cells = [None] * 9
        for word in numeric:
            column = min(range(9), key=lambda i: abs(word["x"] - centers[i]))
            ...
            records.append(
                {
                    ...
                    "sex_label_raw": SEX[column // 3],
                    "age_group_order": column % 3 + 1,
                    "age_group_raw": AGE[column % 3],
                    "percentage_raw": raw,
                    "percentage": Decimal(raw) if raw is not None else None,
                    "measure": "reported_age_distribution_of_elected_officeholders",
        ...
        checks.append(
            {
                ...
                "category_raw": category,
                "sex_label_raw": sex,
                "check": "three_reported_age_percentages_sum_to_100",
                "status": status,
...
def main():
    ...
    for filename, spec in SPECS.items():
        ...
        with tempfile.TemporaryDirectory(prefix="up-age1995-") as temporary:
    ...
    artifact = args.output / "reported_age_percentages.parquet"
    ...
    receipt = {
        ...
        "scope": (
            "Historical aggregate age percentages, not identified seat or candidate"
            " records."
        ),
        "blank_policy": (
    ...
    dictionary = {
        "unit": (
            "One reported age-percentage cell or blank for a post/category/sex group."
        ),
        "percentage": "Decimal 0..100 percentage; null denotes a blank source cell.",
        "age_group_raw": (
            "Printed age headings, including Avobe; no boundary recoding or"
            " disjointness claim."
        ),
        "age_group_order": (
            "Left-to-right age-column ordinal within the printed sex group."
        ),
        "sex_label_raw": (
        ...
        "arithmetic": (
            "Age-group sum checks only. Missing cells prevent comparison; differences"
            " never trigger corrections."
        ),
        "geography": (
